<!doctype html>
<html lang="en">
	<head>
		<meta	charset="UTF-8">
		<title>User Story</title>
		<h1>User Story VZW.Com</h1>
		<div>
			<p>
				Kristi logs into her Verizon Wireless account and is shown the amount of her bill.
			</p>
			<h2>Kristi wants to complete all her account maintenance online.</h2>
			</p>
		</div>
		<div>
			<p>
				<ul>
				<li>Kirist navigates to the site VZW.com and consults the password spreadsheet she created last month for her login info.</li>
				<li>Kristi clicks the sign in button on the top right of the page</li>
				<li>Kristi chooses My Account</li>
				<li>Kristi enters her log in credentials</li>
				<li>Kristi is taken to the account main screen which shows the amount due for her current bill</li>
			</ul>
			</p>
		</div>
	</head>
</html>